...
void run(int argc, char** argv)
{
    ...
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    {
        ...
        // double total_gb = sizeof(unsigned int) * n / 1024. / 1024. / 1024.;
        std::cout << "PCI-E throughput :" << total_bytes / 1024. / 1024. / 1024. / stats::median(times) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        // ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n / LOAD_K_VALUES_PER_ITEM), input_gpu, reduction_buffer1_gpu, reduction_buffer2_gpu);
                        // unsigned mode = GROUP_SIZE;
                        uint work_size = n;
                        auto* in = &input_gpu;
                        auto* out = &reduction_buffer1_gpu;

                        while (work_size > 1) {
                            uint groups = div_ceil(work_size, 0u + GROUP_SIZE);
                            uint work_size_ceiled = groups * GROUP_SIZE;

                            ocl_sum04LocalReduction.exec(
                                gpu::WorkSize(GROUP_SIZE, work_size_ceiled),
                                *in, *out, work_size
                            );
