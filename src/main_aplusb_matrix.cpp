...
void run(int argc, char** argv)
{
...

    {
        ...
        double total_gb = sizeof(unsigned int) * 3 * width * height / 1024. / 1024. / 1024.;
        std::cout << "bad: a + b kernel throughput VRAM: " << total_gb / stats::median(times) << " GB/s" << std::endl;
...

    {
        ...
        // rassert(false, 54623414231);
        double total_gb = sizeof(unsigned int) * 3 * width * height / 1024. / 1024. / 1024.;
        std::cout << "good: a + b kernel throughput VRAM: " << total_gb / stats::median(times) << " GB/s" << std::endl;
        ...
        // Сверяем результат
        for (size_t i = 0; i < width * height; ++i) {
            rassert(cs[i] == as[i] + bs[i], 1832230365731436, cs[i], as[i] + bs[i], i);
        }
    }
}
